...
//typedef vector<pair<int, int> >::const_iterator adj_list_it_t;
...
class Graph
{
    ...
    //Removes edge (u, v) to graph.
    void set_edge(int u_id, int v_id, int val)
    {
        list<pair<int, int> >::iterator it;
        for(it=this->nodes[u_id].begin(); it!=this->nodes[u_id].end(); ++it)
            if((*it).first == v_id)
...
/*class Graph
{
    ...
    {;}
    Graph(int n_nodes):
        nodes(vector<vector<pair<int, int> > >(n_nodes,
            vector<pair<int, int> >(n_nodes, make_pair(0, 0))))
    {;}
    //destructor
    ~Graph()
    {;}
    ...
    void add_edge(int u_id, int v_id, int val)
    {
        this->nodes[u_id][v_id].first = 1;
        this->nodes[u_id][v_id].second = val;
    }

    //Removes edge (u, v) to graph.
    void del_edge(int u_id, int v_id)
    {
        this->nodes[u_id][v_id].first = 0;
    }

    //Returns true iff edge (u, v) exists in graph.
    bool has_edge(int u_id, int v_id) const
    {
        return (bool)this->nodes[u_id][v_id].first;
    }

    //Gets cost of edge (u, v).
    int get_edge_cost(int u_id, int v_id) const
    {
        return this->nodes[u_id][v_id].second;
    }
...
BFSResults bfs(const Graph& graph, int src_id, int dst_id)
{
    queue<int> q;
    vector<bool> visited(graph.n_nodes(), false);
    BFSResults res( vector<int>(graph.n_nodes(), NONE));
...
void edmonds_karp(Graph& flow, const Graph& capacities, int src_id, int dst_id)
{
    ...
    {
        begin = clock();
        BFSResults bfs_res = bfs(res_net, src_id, dst_id);
        end = clock();
        bfs_t += end - begin;
...
Graph reduce(const vector<vector<bool> >& mat)
{
            ...
            for(int i_shift=-1; i_shift<2; i_shift++)
                for(int j_shift=-1; j_shift<2; j_shift++)
                {
                    ...
                    if(mat[i2][j2])
                    {
                        ...
                        graph.add_edge(1+map[i2][j2], 1+map[i][j]+n_true, 1);
                        cap++;
                    }
                }
    ...
    for(int i=0; i<n_true; i++)
        graph.add_edge(1+n_true+i, graph.n_nodes()-1, 1);

    return graph;
}

int all_ge(const vector<int> vec, int n)
{
    int summ = 0;
...
int sol_to_sol(Graph& flow, const Graph& cap)
{
    ...
    vector<int> freq(n_trues, 0);

    for(int u_id=1; u_id<1+n_trues; u_id++)
    {
        sol += flow.adj_list_size(u_id);
        ...
        /*for(int v_id=1+n_trues; v_id<1+2*n_trues; v_id++)
        {
            ...
            {
                freq[u_id-1] += 1;
                freq[v_id-n_trues-1] += 1;
                cout << "opa: " << u_id-1+1 << ", " << v_id-n_trues-1+1 << endl;
            }
        }*/
    }

    //return n_trues - (sol + reps/2);
    return n_trues - sol/2;
}
...
int main()
{
    ...
    */
    ...
    double total = double(bfs_t + ek_t + res_net_t + res_cap_t + augment_t
        + reduction_t + sol_t);

    /*cout << "bfs_t: " << (double(bfs_t)/total)*100 << "%" << endl;
    cout << "ek_t: " << (double(ek_t)/total)*100 << "%" << endl;
    cout << "res_net_t: " << (double(res_net_t)/total)*100 << "%" << endl;
    cout << "res_cap_t: " << (double(res_cap_t)/total)*100 << "%" << endl;
    cout << "augment_t: " << (double(augment_t)/total)*100 << "%" << endl;
    cout << "reduction_t: " << (double(reduction_t)/total)*100 << "%" << endl;
    cout << "sol_t: " << (double(sol_t)/total)*100 << "%" << endl;*/
